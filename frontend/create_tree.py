...
import { Building2, Network, Globe, MapPin, Link as LinkIcon, AlertCircle, Briefcase } from 'lucide-react';
...
const CorporateStructureTree = ({ prospect }) => {
  ...
  const MiniCard = ({ company, badgeLabel, badgeColor }) => (
    <Link to={`/prospects/${company.id}`} className="block p-4 rounded-lg bg-slate-800/50 border border-slate-700 hover:border-slate-500 hover:bg-slate-800 transition-all group">
      ...
      <h4 className="text-sm font-bold text-slate-200 group-hover:text-white mb-1 truncate">{company.company_name}</h4>
      <div className="text-xs text-slate-500 truncate">{company.primary_industries}</div>
    </Link>
  );
